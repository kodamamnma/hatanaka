...
## 2026-09-25 — トップの実績カウンターを削除し、数値は目標として README に移した

### 背景
トップの「120+ 集まった住民の声 / 45+ 記事化実績 / 98% 満足・納得度」は実績がまだ無い数字だった。サイト上に実績として載せるのをやめ、目標値として管理する（クライアント判断）。

### 変更内容

| ファイル | 変更内容 |
|---|---|
| `php/page-top.php` | `.social-proof-bar` セクションを削除 |
| `php/style.css` | `.social-proof-bar` / `.sp-item` / `.sp-num` / `.sp-lbl` と、600px以下の上書きを削除 |
| `.claude/README.md` | 「2. サイトの目的」に目標値の表を追加。コンポーネント一覧から実績カウンターを削除 |

### 未解決・確認事項
- 満足・納得度 98% を測る手段が無い（README の TODO に記載）

---

## 2026-09-25 — 案件用の CLAUDE.md・README.md を作成

### 背景
この案件には `.claude/` に history.md しか無く、作業ルールと案件仕様（目的・トークン・ブレークポイント・デプロイ手順）の置き場が無かった。他案件（med8japan）と同じ形にそろえる。

### 変更内容

| ファイル | 変更内容 |
|---|---|
| `.claude/CLAUDE.md` | 新規。全案件共通ルール（`HP/.claude/CLAUDE.md`）をそのままコピー |
| `.claude/README.md` | 新規。共通の雛形の全項目を、要件定義書（`page-requirements.php`）・テーマのコード・GitHub Actions・この履歴から埋めた |

### 未解決・確認事項
- 本番URL・担当者連絡先・GA4 は情報が無く `未定` のまま
- ブレークポイントは既存の 600px / 960px を正とした（既存CSSはPC基準、新規はモバイルファースト）

---

## 2026-09-25 — 「声を届けるチャット」を全ページ右下から開けるようにした

### 背景
9/24 に作成した声を届けるチャット（`voice-chat.js`）は /submit/ ページ内だけで動いていた。記事を読んでいる途中でもその場で声を届けられるよう、全ページに呼び出しボタンを置く。チャットは既存フォームを埋めて送信する仕組みのため、各ページにもフォームが必要になる。

### 変更内容

| ファイル | 変更内容 |
|---|---|
| `php/voice-form.php` | 新規。投稿フォームを /submit/ から切り出した共通部品（項目を変えるときはここだけ直す） |
| `php/page-submit.php` | フォーム部分を `get_template_part('voice-form')` に置き換え（表示・動作は変更なし） |
| `php/footer.php` | 右下の「声を届ける」ボタンとチャットパネル（`.vc-float`）を追加。/submit/ ではページ内にチャットがあるので出さない |
| `php/functions.php` | voice-chat.css/js を全ページで読み込むように変更（ver 1.1） |
| `php/voice-chat.js` | `initFloat()` を追加。パネルは初めて開いたときに会話を開始。Esc・×で閉じる。送信後に戻ってきたページではパネルを自動で開いてお礼を表示 |
| `php/voice-chat.css` | パネル用スタイルを追加。スマホは全画面、601px以上は右下 400px のカード。トップページのスマホ追従CTAとは重ならないよう上にずらす |

### 未解決・確認事項
- 実機（WordPress）での送信テストは未実施（静的HTMLでの表示確認のみ）
- この案件には `README.md` が無い（構成表・ブレークポイントの定義先が無いため、style.css の 600px に合わせた）
- 9/24 のチャット作成分の履歴エントリが未記入
